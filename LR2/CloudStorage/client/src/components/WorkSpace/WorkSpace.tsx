...
import DirService from "../../API/DirService";
...
const WorkSpace = ({ theme, changeTheme }: Props) => {
  ...
  const [fetchData, isDataLoading, loadingError] = useFetching(
    async (filePath) => {
      const response = await FileService.getFiles(filePath);
      setData(response.data);
      response.data.path !== "/" ? setIsInFolder(true) : setIsInFolder(false);
    }
  );

  const [fetchDownloadFile, isDownloading, downloadError] = useFetching(
    async (downloadFile, filePath) => {
      const response = await FileService.downloadSingleFile(
        downloadFile,
        filePath
      );
    }
  ...
  const [fetchUploadFile, isUploading, uploadError] = useFetching(
    async (fileToSend, filePath) => {
      const response = await FileService.uploadSingleFile(fileToSend, filePath);
      fetchData(filePath);
    }
  );

  const [fetchRemoveItem, isRemoving, removeError] = useFetching(
    async (itemToRemove, filePath) => {
      const response = await DirService.removeItem(itemToRemove, filePath);
      fetchData(filePath);
    }
  );

  const [fetchNewFolder, isCreating, createError] = useFetching(
    async (folderName, filePath) => {
      const response = await DirService.newFolder(folderName, filePath);
      fetchData(filePath);
    }
  );

  const [fetchSearchedItem, isSearching, searchError] = useFetching(
    async (itemToSearch) => {
